    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      >
        {submitting ? (
          ...
          <>
            <IconPlus />
            <span>Create Fellowship Group</span>
          </>
        )}
      </button>
    </form>
...
function EditFellowshipForm({
  ...
  const updateFellowship = useMainStore((state) => state.updateFellowship);
  const [name, setName] = useState(group.name || "");
  const [description, setDescription] = useState(group.description || "");
  const [leaderId, setLeaderId] = useState(
    group.leader ? String(group.leader) : "",
  );
  const [memberSearch, setMemberSearch] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) return setError("Group name is required.");
    setSubmitting(true);
    const result = await updateFellowship(group.id, {
      name,
      description,
      leader: leaderId ? Number(leaderId) : null,
      members: selectedMembers,
    ...
    setSubmitting(false);
    if (result?.success) onSuccess?.(result.group);
    else setError("Failed to update group. Please try again.");
  };
          ...
          {filteredUsers.map((u) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`w-4 h-4 border flex items-center justify-center shrink-0 ${checked ? "bg-amber-500 border-amber-500" : "border-stone-300"}`}
                >
                  {checked && (
                    <svg
                      className="w-2.5 h-2.5 text-white"
...
function Fellowships() {
  ...
  const deleteFellowship = useMainStore((state) => state.deleteFellowship);

  const fellowshipList =
    fellowships?.results ?? (Array.isArray(fellowships) ? fellowships : []);
  ...
  const handleDelete = async (id) => {
    const result = await deleteFellowship(id);
    if (result?.success) {
      setSelected(null);
      setMode("idle");
      showSuccess("Fellowship group deleted.");
    }
  };
  const handleEditSuccess = (updatedGroup) => {
    setSelected(updatedGroup);
    setMode("idle");
    showSuccess("Fellowship group updated successfully ✓");
  };
